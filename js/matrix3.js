class Matrix3 {
    ...
    static rotation(theta, point) {
        ...
        if (point === undefined)
            return r;
        let move = Matrix3.identity();
        move.x3 = point.x;
        move.y3 = point.y;
        let unmove = Matrix3.identity();
        unmove.x3 = -point.x;
        unmove.y3 = -point.y;
        r.mul_left(move);
        unmove.mul_left(r);
    ...
    rotate(theta, point) {
        this.mul_left(Matrix3.rotation(theta, point));
    }

    rotate_rel(theta) {
        this.mul_left(Matrix3.rotation(theta, new Vector2(this.x3, this.y3)));
    }
